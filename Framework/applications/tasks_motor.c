...
float g_yawAngleTarget = 0.0;
float g_pitchAngleTarget = 0.0;
...
//		       g_ChassisSpeedRef.rotate_ref = CMRotatePID.output;
//					fw_printfln("CMRotatePID.output:%f",CMRotatePID.output);
...
/******发送数据1  yaw角度*******/
	...
	/*底盘跟随编码器旋转PID计算*/		
	   ...
	   CMRotatePID.Calc(&CMRotatePID);   
		...
		//fw_printfln("CMRotatePID.output:%f",CMRotatePID.output);
//				g_ChassisSpeedRef.rotate_ref = 0;
...
/*自瞄模式切换*/
			if(GetShootMode() == AUTO) {
				...
				g_yawAngleTarget = yawRealAngle - yawAdd ;
...
//大神符
					...
					if(GetRuneState() == AIMING){
						...
						g_yawAngleTarget = Location_Number[rune - 1].yaw_position;
						rune_flag--;
					}
				}
		  }
...
//			MINMAX(g_yawAngleTarget, -45, 45);
			yawIntensity = PID_PROCESS_Double(yawPositionPID,yawSpeedPID,g_yawAngleTarget + sum_flag + sum_flag1,yawRealAngle,-gYroZs);
...
//自瞄模式切换
			if(GetShootMode() == AUTO) 
			  ...
			  g_pitchAngleTarget = pitchRealAngle + pitchAdd ;
				CReceive --;
				}
//大神符
				else if((GetLocateState() == Located) && (rune_flag != 0)){
					if(GetRuneState() == AIMING){
						g_pitchAngleTarget = Location_Number[rune - 1].pitch_position;
						rune_flag--;
...
#ifdef Infantry_2
			MINMAX(g_pitchAngleTarget, -10.0f, 26.3f);
#endif
#ifdef Infantry_3
			MINMAX(g_pitchAngleTarget, -18.f, 30);
#endif
#ifdef Infantry_4
			MINMAX(g_pitchAngleTarget, -9.0f, 32);
			//原本想避免弹道卡住装甲板
//float Pitch_add = 0;			
...
//}
#endif
//			MINMAX(g_pitchAngleTarget, -28.f, 26);

			pitchIntensity = PID_PROCESS_Double(pitchPositionPID,pitchSpeedPID,g_pitchAngleTarget,pitchRealAngle,-gYroXs);
			//		fw_printfln("pitchIntensity:%d", pitchIntensity);
			setMotor(GMPITCH, pitchIntensity);
		}
//底盘电机 1 2 3 4	
//		g_ChassisSpeedRef.rotate_ref = 0;//取消底盘跟随
		...
		if(IOPool_hasNextRead(CMFLRxIOPool, 0)){
			...
			CM2SpeedPID.ref =  g_ChassisSpeedRef.forward_back_ref*0.075 + g_ChassisSpeedRef.left_right_ref*0.075 + g_ChassisSpeedRef.rotate_ref;
...
#ifdef Infantry_1_Aim	
			CANReceiveMsgProcess_820R(pData, &CM1Encoder);				
		  CM1SpeedPID.ref =  -g_ChassisSpeedRef.forward_back_ref*0.075 + g_ChassisSpeedRef.left_right_ref*0.075 + g_ChassisSpeedRef.rotate_ref;
...
//		 fw_printfln("CM1Encoder.filter_rate:%d",CM1Encoder.filter_rate);
		...
		if(IOPool_hasNextRead(CMBLRxIOPool, 0)){
			...
			CM3SpeedPID.ref =  g_ChassisSpeedRef.forward_back_ref*0.075 - g_ChassisSpeedRef.left_right_ref*0.075 + g_ChassisSpeedRef.rotate_ref;
		...
		if(IOPool_hasNextRead(CMBRRxIOPool, 0)){
			...
			CM4SpeedPID.ref =  -g_ChassisSpeedRef.forward_back_ref*0.075 - g_ChassisSpeedRef.left_right_ref*0.075 + g_ChassisSpeedRef.rotate_ref;
