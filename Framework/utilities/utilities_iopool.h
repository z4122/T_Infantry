/**
  ...
  * IOPOOL 
	...
	*
	* 使用方法：
	* 线程1使用IOPool_getNextWrite(ioPool)( IOPool_pGetWriteData(ioPool)? )获得指向WritePool的指针并修改WritePool
  * 线程1修改WritePool完成后调用IOPool_getNextWrite(ioPool)，将WritePool数据复制到ExchangePool
	...
	* 若检测到有数据，线程2使用IOPool_has(get?)NextRead(ioPool, id)将ExchangePool数据复制到ReadPool
